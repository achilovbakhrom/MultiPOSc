...
public class CustomerExportDialog extends Dialog {
    ...
    public CustomerExportDialog(@NonNull Context context, DatabaseManager databaseManager) {
        super(context);
        ...
        String[] items = {context.getString(R.string.customers), context.getString(R.string.template)};
        ...
        btnExport.setOnClickListener(view -> {
            if (fromUsb) {
                ...
                switch (pos) {
                    case 0:
                        if (docType == EXCEL) {
                            ExportUtils.exportCustomersToUsb(context, root, databaseManager);
                        } else {
                            ExportUtils.exportCustomersPdfToUsb(context, root, databaseManager);
                        }
                        break;
                    case 1:
                        ExportUtils.exportCustomersTemplateToUsb(context, root);
                        break;
                }
                dismiss();
            } else {
                if (!tvFilePath.getText().toString().isEmpty()) {
                    switch (pos) {
                        case 0:
                            if (docType == EXCEL) {
                                ExportUtils.exportCustomers(context, path, databaseManager);
                            } else {
                                ExportUtils.exportCustomersPdf(context, path, databaseManager);
                            }
                            break;
                        case 1:
                            ExportUtils.exportCustomersTemplate(context, path);
                            break;
                    }
                    dismiss();
